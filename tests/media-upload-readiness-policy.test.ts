...
describe('media upload readiness policy guardrail', () => {
  ...
  it('detects current safe upload foundations and missing variant generation', async () => {
    ...
    assert.equal(readiness.checks.hasCentralLimitPolicy, true)
    assert.equal(readiness.checks.hasVariantIntentPolicy, true)
    assert.equal(readiness.checks.hasStorageBoundaryPolicy, true)
    assert.equal(readiness.checks.supportsNextImageFormats, true)
    assert.equal(readiness.checks.objectStorageImplemented, false)
