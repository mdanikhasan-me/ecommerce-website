...
describe('Step 312 desktop navbar categories dropdown redesign', () => {
  ...
  it('keeps desktop dropdown click toggle, keyboard dismissal, and inside-safe category switching wired', () => {
    assert.match(headerSource, /const toggleCategoriesDropdown = \(\) =>/)
    assert.match(headerSource, /onClick=\{toggleCategoriesDropdown\}/)
    assert.match(headerSource, /setIsCategoriesOpen\(\(open\) => !open\)/)
    ...
    assert.doesNotMatch(headerSource, /onClick=\{openCategoriesDropdown\}/)
    assert.doesNotMatch(headerSource, /onFocus=\{openCategoriesDropdown\}/)
    assert.doesNotMatch(headerSource, /onBlur=\{\(event\) =>/)
  })
  ...
  it('keeps the Step 313 dropdown calmer and does not select categories on hover', () => {
    ...
    assert.match(headerSource, /h-\[25rem\]/)
    assert.match(headerSource, /max-h-\[calc\(100vh-7rem\)\]/)
    assert.match(headerSource, /min-h-\[18rem\]/)
    assert.match(headerSource, /grid h-full min-h-\[18rem\]/)
    assert.match(headerSource, /overflow-y-auto border-r/)
    assert.match(headerSource, /min-h-0 overflow-hidden bg-\[#fffdfa\]/)
    assert.match(headerSource, /grid auto-rows-fr grid-cols-5/)
  ...
  it('keeps profile avatars display-ready with a default icon fallback', () => {
    assert.match(headerSource, /function HeaderAvatar/)
    assert.match(headerSource, /const normalizedImageUrl = imageUrl\?\.trim\(\) \?\? ''/)
    assert.match(headerSource, /const \[hasImageError, setHasImageError\] = useState\(false\)/)
    assert.match(headerSource, /onError=\{\(\) => setHasImageError\(true\)\}/)
    assert.match(headerSource, /return <LocalIcon name="user" className=\{iconClassName\} \/>/)
    assert.match(headerSource, /<HeaderAvatar imageUrl=\{session\.user\.image\} \/>/)
    assert.match(headerSource, /<HeaderAvatar imageUrl=\{session\?\.user\.image\} \/>/)
  })
})
