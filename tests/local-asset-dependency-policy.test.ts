...
describe('local asset dependency policy', () => {
  ...
  it('keeps static UI assets local or bundled and separates remote catalog media backlog', async () => {
    ...
    assert.equal(audit.productSourceAssetFolder.exists, true)
    assert.equal(audit.productSourceAssetFolder.fileCount > 0, true)
    assert.equal(audit.productSeedMedia.productSeedLocalProductSourceAssetCount > 0, true)
    ...
    assert.equal(evidence.sourceUploadBoundary.productSeedUsesProductSourceAssetCount > 0, true)
