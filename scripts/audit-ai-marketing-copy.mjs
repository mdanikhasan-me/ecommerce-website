...
const DEFAULT_ROOTS = ['README.md', 'src', 'prisma/seed.ts', 'public/assets/README.md']
...
export function findMarketingCopyFindings(text, filePath = 'inline') {
  ...
  for (const [index, line] of lines.entries()) {
    if (/trusted-fetch-site|TRUSTED_FETCH|AUTH_TRUST_HOST/.test(line)) continue
...
async function main() {
  ...
  for (const finding of result.findings.slice(0, 50)) {
    console.log(`${finding.severity}\t${finding.file}:${finding.line}\t${finding.id}\t${finding.excerpt}`)
  }
}
