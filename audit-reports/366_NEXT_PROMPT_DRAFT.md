# Step 367 Next Prompt Draft

Continue after Step 366 in the Boilabin project.

Step 366 skipped the blocked media decision and hardened safe callback URL targets so login/register callbacks cannot point to auth loops, API/framework routes, or static/upload assets.
...
If skipping media, choose one bounded non-media task, add focused tests if code changes, create a Step 367 report/evidence folder, validate, stage exact files only, commit, then push `origin main`.
...
1. Summary of the Step 367 decision/work
