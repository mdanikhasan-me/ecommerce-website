# Step 371 Next Prompt Draft

Continue after Step 370 in the Boilabin project.

Step 370 skipped the blocked media decision and hardened NextAuth redirect callback handling so callback URL query/cookie redirects use a backend same-origin page target allowlist instead of default same-origin behavior.
...
If skipping media, choose one bounded non-media task, add focused tests if code changes, create a Step 371 report/evidence folder, validate, stage exact files only, commit, then push `origin main`.
...
1. Summary of the Step 371 decision/work
