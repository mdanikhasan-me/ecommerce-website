# Step 370 Next Prompt Draft

Continue after Step 369 in the Boilabin project.

Step 369 skipped the blocked media decision and hardened auth host origin parsing so only HTTP(S) origins count as canonical auth origins.
...
If skipping media, choose one bounded non-media task, add focused tests if code changes, create a Step 370 report/evidence folder, validate, stage exact files only, commit, then push `origin main`.
...
1. Summary of the Step 370 decision/work
