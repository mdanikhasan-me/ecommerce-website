# Step 373 Next Prompt Draft

Continue after Step 372 in the Boilabin project.

Step 372 skipped the blocked media decision and hardened auth host origin parsing so URL userinfo credentials are rejected before a configured auth origin can be treated as canonical.
...
If skipping media, choose one bounded non-media task, add focused tests if code changes, create a Step 373 report/evidence folder, validate, stage exact files only, commit, then push `origin main`.
...
1. Summary of the Step 373 decision/work
