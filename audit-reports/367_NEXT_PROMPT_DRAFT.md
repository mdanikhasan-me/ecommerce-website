# Step 368 Next Prompt Draft

Continue after Step 367 in the Boilabin project.

Step 367 skipped the blocked media decision and hardened safe callback URL targets so login/register callbacks cannot point to exact metadata/icon routes.
...
If skipping media, choose one bounded non-media task, add focused tests if code changes, create a Step 368 report/evidence folder, validate, stage exact files only, commit, then push `origin main`.
...
1. Summary of the Step 368 decision/work
