# Step 372 Next Prompt Draft

Continue after Step 371 in the Boilabin project.

Step 371 skipped the blocked media decision and hardened auth redirect helper handling so URL userinfo credentials are rejected in both base URLs and candidate redirect targets.
...
If skipping media, choose one bounded non-media task, add focused tests if code changes, create a Step 372 report/evidence folder, validate, stage exact files only, commit, then push `origin main`.
...
1. Summary of the Step 372 decision/work
