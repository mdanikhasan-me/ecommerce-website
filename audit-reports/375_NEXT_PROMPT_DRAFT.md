# Step 376 Next Prompt Draft

Continue after Step 375 in the Boilabin project.

Step 375 skipped the blocked media decision and hardened SEO URL helpers so credential-bearing URL userinfo is rejected instead of being canonicalized into site, canonical, media, Open Graph, sitemap, or JSON-LD URL fields.
...
If skipping media, choose one bounded non-media task, add focused tests if code changes, create a Step 376 report/evidence folder, validate, stage exact files only, commit, then push `origin main`.
...
1. Summary of the Step 376 decision/work
