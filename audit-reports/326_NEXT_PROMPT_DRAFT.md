# Step 327 Next Prompt Draft

Continue after Step 326 in the Boilabin project.

Step 326 skipped the blocked media decision and broadened Advisor's private env file read guard so `readSafeFile` refuses every `.env` and `.env.*` filename variant before attempting to read.
...
If skipping media, choose one narrow non-media task, add focused tests if code changes, create a Step 327 report/evidence folder, validate, and stage exact files only.
...
1. Summary of the Step 327 decision/work
