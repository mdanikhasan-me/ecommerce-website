# Step 364 Next Prompt Draft

Continue after Step 363 in the Boilabin project.

Step 363 skipped the blocked media decision and hardened CSP pathname normalization so query strings, hashes, full URLs, relative paths, and blank input are normalized before route classification.
...
If skipping media, choose one bounded non-media task, add focused tests if code changes, create a Step 364 report/evidence folder, validate, stage exact files only, commit, then push `origin main`.
...
1. Summary of the Step 364 decision/work
