# Step 365 Next Prompt Draft

Continue after Step 364 in the Boilabin project.

Step 364 skipped the blocked media decision and hardened middleware session-cookie hint matching so only exact known cookie names or numeric Auth.js chunk suffixes count as session-cookie hints.
...
If skipping media, choose one bounded non-media task, add focused tests if code changes, create a Step 365 report/evidence folder, validate, stage exact files only, commit, then push `origin main`.
...
1. Summary of the Step 365 decision/work
