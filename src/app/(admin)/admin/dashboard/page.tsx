      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {data.pendingOrders > 0 && (
            <Link href="/admin/orders?status=PENDING" className="flex items-center gap-2 bg-amber-50 text-amber-700 border border-amber-200 rounded-md px-3 py-2 text-sm font-medium min-[1025px]:hover:bg-amber-100 transition-colors">
              <Clock className="h-4 w-4" />
              {data.pendingOrders} pending orders
            </Link>
          )}
          {data.pendingReviews > 0 && (
            <Link href="/admin/reviews?status=PENDING" className="flex items-center gap-2 bg-blue-50 text-blue-700 border border-blue-200 rounded-md px-3 py-2 text-sm font-medium min-[1025px]:hover:bg-blue-100 transition-colors">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-card border border-border rounded-md overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-border">
            ...
            <Link href="/admin/orders" className="text-sm text-primary min-[1025px]:hover:underline">View All</Link>
          </div>
          <div className="divide-y divide-border">
            {data.recentOrders.map((order) => {
              ...
              return (
                <Link key={order.id} href={`/admin/orders/${order.id}`} className="flex items-center gap-3 px-5 py-3 min-[1025px]:hover:bg-secondary transition-colors">
                  <div>
            ...
            <div className="bg-card border border-border rounded-md overflow-hidden">
              ...
              <div className="p-3 space-y-2">
                {data.lowStockProducts.map((p) => (
                  <Link key={p.id} href={`/admin/products/${p.id}`} className="flex items-center justify-between p-2 rounded-md min-[1025px]:hover:bg-secondary transition-colors text-sm">
