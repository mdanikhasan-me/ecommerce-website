  ...
  return (
    <main className="bg-white text-foreground">
      ...
      <div className="container-site py-7 sm:py-9 lg:py-10">
        ...
        <section className="mt-9 grid gap-3 rounded-lg bg-muted/70 p-4 sm:grid-cols-2 sm:p-5 lg:grid-cols-5">{topics.map(([title, description, href, icon, tone]) => <Link key={title} href={href} className="flex min-w-0 items-center gap-4 px-1 py-2"><span className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-full ${tone}`}><LocalIcon name={icon} className="h-7 w-7" /></span><span className="min-w-0 flex-1"><strong className="block text-sm">{title}</strong><span className="mt-1 block text-sm leading-6 text-muted-foreground">{description}</span></span><LocalIcon name="chevron-right" className="h-4 w-4 shrink-0 text-muted-foreground" /></Link>)}</section>
