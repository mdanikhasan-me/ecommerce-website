...
export async function PUT(req: NextRequest, { params }: RouteContext) {
  try {
    ...
    const message =
      error.code === 'P2002' ? 'Coupon code already exists' : error.message || 'Unable to update coupon'
...
export async function DELETE(_req: NextRequest, { params }: RouteContext) {
  try {
    ...
    const status = error.message === 'Unauthorized' ? 401 : 400
    return NextResponse.json({ error: error.message || 'Unable to delete coupon' }, { status })
  }
}
